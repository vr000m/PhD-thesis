...
\citepub{c:err}, \citepub{c:fecrc}, \cite{draft.1d2d.fec}, and \cite{draft.adaptive.fec}.
...
Figure~\ref{fig:fec_fw} shows the sender-side FEC framework and describes
the interaction between the FEC module, FEC scheme, and the RTP stack.
...
In \citepub{c:fecrc}, the sender generates the FEC packets using the 
non-interleaved or interleaved parity codes from a set of source media packets
encapsulated in RTP. The parity codes are a set of systematic codes in which a
number of repair packets are generated from a set of source media packets.
There are several FEC schemes, namely, non-interleaved (1-d) parity,
interleaved (2-d) parity, linear parity codes (ldpc), Reed-Solomon (RS), etc.
In the non-interleaved parity scheme an XOR packet is generated for every  N
source packets. Consequently, this FEC scheme is able to recover from a single
random packet loss in N packets. In addition to the 1-d parity FEC, if we
apply the XOR operation on source packets that are N sequence numbers apart
from each other and in blocks of B packets (multiple of N packets), the
resulting N repair packets are referred to as interleaved (2-d) FEC packets.
This scheme (2-d parity) is able to cope with burst loss of at least N packet
in every B block of packets. Typically, the repair packets are encapsulated in 
an RTP payload format, for example using the packet format described in 
\cite{rfc5109} or \cite{draft.1d2d.fec}.
