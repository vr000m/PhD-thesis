...
intervals; in this paper we use 1\,\emph{s} reporting interval, with early and
...
To the second problem: we propose a congestion control scheme where instead of
...
successfully, the sender increases the sender rate with the amount of FEC
...
variable FEC interval, i.e., it varies the number of packet for which FEC is
generated. Hence, if the sender thinks that it is underutilizing the link by
large margin, it introduces a shorter FEC interval (up to 33\,\% redundancy)
and therefore ramps up quickly. Consequently when it thinks that it is closer
...
This chapter concludes the discussion of in-path congestion control, in the
next two chapters we will discuss about using congestion cues from off-path
sources to perform congestion control.
