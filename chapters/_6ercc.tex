...
recovered for an end-to-end delay of 60ms (meeting the 150ms playout
...
delay scenarios. Similarly, protecting the media flows with UEP incurred a
...
one-way delay is 100 ms and the frame rate is set to 15 frames per second, we
observe the error propagation is stopped by RPS in about 4 to 7 pictures.
...
\begin{tabular}{cccc}
\hline
 & \multicolumn{3}{l}{PSNR After Simulation} \\
\hline
...
\hline
\end{tabular}
\caption{Comparing the performance (Y-PSNR) of different error-resilience
...
observed packet loss and end-to-end delay and figure~\ref{fig:apply_err}
summarizes the applicability of the error-resilience mechanisms based on our
experiments.

\begin{figure}
\centerline {
\includegraphics[width=0.8\textwidth]{chap6-fig-apply-err}
}
