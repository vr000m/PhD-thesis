...
packets for an end-to-end latency of 60\,\emph{ms} (meeting the 150\,\emph{ms}
playout deadline). The number of recovered packets will increase with shorter
RTCP reporting intervals; in this paper we use \emph{1s} reporting interval,
with early and immediate reporting as defined in \cite{rfc4585}. Based on
these experiments, it shows that NACK is an effective mechanism for low
end-to-end latency scenarios. Similarly, protecting the media flows with UEP
incurred a 15-25\,\% overhead, and about 21-24\,\% of the lost packets were
recovered.
...
reports RTCP every 250\,\emph{ms} and it approximately consumes 2\,\% of the
session media rate. In Figure ~\ref{fig:rpsi_sim} (a), (b) and (c) we observe
