...
Figure~\ref{amusys-descr} illustrates the integration of the application,
...
\section{Metrics for Çongestion Control}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{chap4_fig-perf-metrics}
...
rate and goodput. The instantaneous rate is calculated at 1s intervals and the
goodput is calculated after discarding the late packets and before passing the
packets to the decoder.


The Average Bandwidth Utilization (ABU) is the ratio between the instantaneous
goodput (or encoding, receiving bit rate) and the instantaneous channel
capacity. Ratios over ($>100\%$) represent over-utilization and therefore
congestion losses.
...
\begin{figure}[!t]
  \centerline{
    {\includegraphics[width=0.9\textwidth]{chap4_graph_cb_cmp_trr_2s}}
  }
