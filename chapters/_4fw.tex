...
Another aspect to consider is long-term (order of seconds or minutes) and
short-term (order of 100ms or a few seconds) statistics. For example, jitter
is measured on a per-packet basis, but reported over a longer measurement
interval (to filter for noise and transient delays). Alternatively, packet
losses, discards, etc. are measured over a shorter interval so that the sender
can react to these notifications.
