...
playout quickly. It takes some 3\,\emph{s} to adapt the playout while the one
...
path is variable, it demonstrates the sensitivity of the scheduling algorithm
...
instantaneous bandwidth utilization for each MPRTP subflow. In this case on
observing congestion on Path B, the scheduling algorithm reallocate the media
on to the other paths (observe the points where the link rate drops).
...
In this thesis, we did not apply using MPRTP for interactive real-time
