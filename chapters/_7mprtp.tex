...
In Figure~\ref{chap7:fig_sim_var_bw} the Path B's capacity varies while Path
A's capacity remains constant. The figure also shows the instantaneous
bandwidth utilization for each MPRTP subflow. In this case on observing
congestion on Path B, the scheduling algorithm reallocate the media on to the
other paths (observe the points where the link rate drops).  
