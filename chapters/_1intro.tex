...
Adobe Flash Video plugin and later, due to HTML5 video tag). Currently, the
same trend can be observed with interactive multimedia, i.e., web services can
use Adobe's Real-time Media Flow Protocol (RTMFP)~\cite{draft.rtmfp} or
HTML5's Web Real-Time Communication (WebRTC)~\cite{draft.webrtc}.
...
idea was to build a congestion-control that gets insights into the current
network conditions, media codec operation and the demands of the multimedia
application (user's preference, application settings, user's quality of
experience).
