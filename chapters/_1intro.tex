...
This thesis is about enabling congestion control for real-time communication and
...
encoding rate based on the observed congestion cues.
...
rate (for e.g., by an in-path device, or a 3G base station). \citepub{c:hetrc}
...
In \citepub{c:mprtp}, we enable multi-homing for real-time flows and extend the
...
their media throughput and geo-location to a network coverage map service, which
...
draft.rmcat.evaluate, Singh:PhDFw, draft.rtp.cb}, and \citepub{c:cb}.
